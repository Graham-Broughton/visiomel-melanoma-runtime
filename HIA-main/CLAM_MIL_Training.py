    ...
    for targetLabel in targetLabels:
        for repeat in range(args.repeatExperiment):
            ...
            args.projectFolder = utils.CreateProjectFolder(
                args.project_name, args.adressExp, targetLabel, args.model_name, repeat + 1
            )
            ...
            if args.extractFeature:
                ...
                ExtractFeatures(
                    data_dir=imgs, feat_dir=args.feat_dir, batch_size=args.batch_size, target_patch_size=-1, filterData=True
                )
            ...
            if args.train_full:
                ...
                if args.early_stopping:
                    ...
                    df = pd.DataFrame(
                        {'train': pd.Series(train_data['PATIENT']), 'test': pd.Series([]), 'val': pd.Series(val_data['PATIENT'])}
                    )
                    ...
                    train_dataset, val_dataset, test_dataset = dataset.Return_splits(
                        from_id=False, csv_path=os.path.join(args.split_dir, 'TrainSplit.csv')
                    )
                else:
                    ...
                    train_dataset, val_dataset, test_dataset = dataset.Return_splits(
                        from_id=False, csv_path=os.path.join(args.split_dir, 'TrainValSplit.csv')
                    )
                ...
                for train_index, test_index in kf.split(patientID, yTrue):
                    ...
                    train_data = pd.DataFrame(
                        list(zip(trainPatients, trainyTrue, trainyTrueLabel)), columns=['PATIENT', 'yTrue', 'yTrueLabel']
                    )
                    ...
                    test_data = pd.DataFrame(
                        list(zip(testPatients, testyTrue, testyTrueLabel)), columns=['PATIENT', 'yTrue', 'yTrueLabel']
                    )
                    train_data.reset_index(inplace=True, drop=True)
                    ...
                    train_dataset, val_dataset, test_dataset = dataset.Return_splits(
                        from_id=False, csv_path=os.path.join(args.split_dir, 'TrainTestValSplit_{}.csv'.format(foldcounter))
                    )
                    datasets = (train_dataset, val_dataset, test_dataset)
                    ...
                    CalculatePatientWiseAUC(
                        resultCSVPath=testResultsPath, args=args, foldcounter=foldcounter, clamMil=True, reportFile=reportFile
                    )
                    reportFile.write('-' * 30 + '\n')
                    foldcounter += 1
