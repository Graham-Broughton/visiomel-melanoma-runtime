...
def Compute_w_loader(
    file_path, output_path, model, batch_size=8, verbose=0, print_every=20, pretrained=True, target_patch_size=-1
):
...
def ExtractFeatures(data_dir, feat_dir, batch_size, target_patch_size=-1, filterData=True):
    ...
    for bag_candidate_idx in range(total):
        ...
        if not os.path.exists(os.path.join(feat_dir, f'{bag_base}.pt')):
            print(bag_name)
            ...
            if os.path.exists(output_file_path):
                ...
                torch.save(features, os.path.join(feat_dir, f'{bag_base}.pt'))
                file.close()
