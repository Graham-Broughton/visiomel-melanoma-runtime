...
def Train_MIL_CLAM(datasets, fold, args, trainFull=False):
    ...
    for epoch in range(args.max_epochs):
        ...
        if args.model_name in ['clam_sb', 'clam_mb'] and not args.no_inst_cluster:
            ...
            Train_loop_MIL(
                epoch=epoch, model=model, loader=train_loader, optimizer=optimizer, n_classes=args.num_classes, loss_fn=loss_fn
            )
            if args.early_stopping:
                stop = Validate_MIL(
                    fold=fold,
    ...
    if not trainFull:
        patient_results, test_error, test_auc = Summary(
            model=model, loader=test_loader, n_classes=args.num_classes, modelName=args.model_name
        )
