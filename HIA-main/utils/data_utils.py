...
def Get_split_loader(split_dataset, training=False, weighted=True):
    ...
    if training:
        if weighted:
            ...
            loader = DataLoader(
                split_dataset, batch_size=1, sampler=RandomSampler(split_dataset), collate_fn=collate_MIL_Training, **kwargs
            )
    else:
        loader = DataLoader(
            split_dataset, batch_size=1, sampler=SequentialSampler(split_dataset), collate_fn=collate_MIL_Testing, **kwargs
        )
    return loader
...
def ConcatCohorts_Classic(
    imagesPath, cliniTablePath, slideTablePath, label, reportFile, outputPath, csvName, patientNumber='ALL', minNumberOfTiles=0
):
    patients = []
    ...
    for imgCounter in range(len(imagesPath)):
        ...
        reportFile.write(
            'Data for {} Patients from Clini Table is not found in Slide Table!'.format(len(inClinicalNotInSlide)) + '\n'
        )
        ...
        reportFile.write(
            'Data for {} Patients from Slide Table is not found in Clini Table!'.format(len(inSlideNotInClinical)) + '\n'
        )
...
def GetTiles(csvFile, label, maxBlockNum, target_labelDict, test=False, seed=23, filterPatients=[]):
    ...
    for index, patientID in enumerate(tqdm(patientsUnique)):
        ...
        for item in range(len(selectedData)):
            tempTiles.extend(
                [os.path.join(selectedData['SlideAdr'][item], i) for i in os.listdir(selectedData['SlideAdr'][item])]
            )
    ...
    df = pd.DataFrame(
        list(zip(patinetList, tilesPathList, yTrueList, yTrueLabelList)), columns=['PATIENT', 'TilePath', 'yTrue', 'yTrueLabel']
    )
    df_temp = df.dropna()
