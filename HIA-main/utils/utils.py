...
def get_optim(model, args, params=False):
    ...
    if args.opt == "adam":
        ...
        optimizer = optim.AdamW(temp, lr=args.lr, momentum=0.9, weight_decay=args.reg)
    elif args.opt == 'lion':
        from models.lion_pytorch.lion_pytorch import Lion
        optimizer = Lion(temp, lr=1e-5, weight_decay=1e-2)
    else:
