...
class Generic_WSI_Classification_Dataset(Dataset):
    def __init__(self, csv_path='dataset_csv/ccrcc_clean.csv', shuffle=False, seed=7, print_info=True, label_dict=None, filter_dict=None, ignore=None, patient_strat=False, label_col=None, patient_voting='max'):
        """
        Args:
    ...
    def create_splits(
        self, k=3, val_num=(25, 25), test_num=(40, 40), label_frac=1.0, custom_test_ids=None, test_same_as_val=False
    ):
        settings = {
            'n_splits': k,
            'val_num': val_num,
        ...
        if self.patient_strat:
            ...
            settings |= {'cls_ids': self.slide_cls_ids, 'samples': len(self.slide_data)}
    ...
    def return_subset(self, slide_ids):
        ...
        return self.Generic_Split(
            data, data_dir=self.data_dir, num_classes=self.num_classes
        )
    ...
    def test_split_gen(self, return_descriptor=False, allow_val_test_overlap=False):
        ...
        for u in range(len(unique)):
            print(f'number of samples in cls {unique[u]}: {counts[u]}')
            if return_descriptor:
                df.loc[index[u], 'train'] = counts[u]

        count = len(self.val_ids)
        print(f'\nnumber of val samples: {count}')
