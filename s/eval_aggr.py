...
class CLAMTrainFoldsDir(object):
            ...
            if os.path.isdir(fold_dp):
                ...
                if len(time_dirs) > 0:
                    if fold_dp.parts[-1] in force_time_dir:
                        ...
                        self.__extracted_from___setup_config_dict_14(
                            "using forced time_dir:",
                            time_dn,
                            fold_dp,
                            fold_train_dir_dict,
                        )
    ...
    def __extracted_from___setup_config_dict_14(self, arg0, arg1, fold_dp, fold_train_dir_dict):
        print(arg0)
        print(arg1)
        fold_train_dir_dict[fold_dp.parts[-1]] = join(
            self.train_folds_dir, fold_dp.parts[-1], arg1
        )
    ...
    def compute_aggr_eval_metrics(self):
        eval_detailed_df_list = []
        eval_summary_dict_list = []
        ...
        if eval_summary_dict_list[0]["result_type"] == "multiclass_classification":
            self._extracted_from_compute_aggr_eval_metrics_19(
                eval_summary_dict_list, df, micro_acc, macro_acc
            )
        elif eval_summary_dict_list[0]["result_type"] == "binary_classification":
            self._extracted_from_compute_aggr_eval_metrics_44(
                eval_summary_dict_list, df, micro_acc, macro_acc
            )
        self.__setup_config_dict(self.force_time_dir)

    def _extracted_from_compute_aggr_eval_metrics_44(self, eval_summary_dict_list, df, micro_acc, macro_acc):
        assert all(
            summary_dict["result_type"] == "binary_classification"
            for summary_dict in eval_summary_dict_list
        )
    ...
    def _extracted_from_compute_aggr_eval_metrics_19(self, eval_summary_dict_list, df, micro_acc, macro_acc):
        assert all(
            summary_dict["result_type"] == "multiclass_classification"
            for summary_dict in eval_summary_dict_list
        )
        auc_class_micro_fold_macro = np.mean(
            [summary_dict["eval_scores"]["auc_roc_class_micro"] for summary_dict in eval_summary_dict_list]
        )
        auc_class_macro_fold_macro = np.mean(
            [summary_dict["eval_scores"]["auc_roc_class_macro"] for summary_dict in eval_summary_dict_list]
        )
