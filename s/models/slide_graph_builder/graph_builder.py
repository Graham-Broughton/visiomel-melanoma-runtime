...
class SlideGraph(object):
    def __init__(self, slide_features, slide_coords, nx_graph, patches_attrs):
        self.slide_features, self.slide_coords, self.nx_graph, self.patches_attrs = (
            slide_features,
            slide_coords,
            nx_graph,
            patches_attrs,
            ...
            pkl.dump(
                {
                    ...
                    "floyd_warshall": {"M": self.M, "Pred": self.Pred},
                },
                file=f,
            )

    @staticmethod
    def build_slide_graph(patch_h5_dir, feature_h5_dir, slide_id, patch_dir_name, feature_dir_name, k=8):
        h5_fp = join(patch_h5_dir, f"{slide_id}.h5")
        with h5py.File(h5_fp, 'r') as f:
            patches_attrs = dict(f["coords"].attrs)
        h5_fp = join(feature_h5_dir, f"{slide_id}.h5")
        with h5py.File(h5_fp, 'r') as f:
            features = f['features'][()]
            coords = f['coords'][()]
        ...
        return SlideGraph(features, coords, nx_graph, patches_attrs)
