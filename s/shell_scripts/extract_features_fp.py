...
import argparse
from utils.utils import collate_features, device
...
def compute_w_loader(
    ...
    for count, (batch, coords) in enumerate(loader):
        toc = time.time()
        print(f"batch {count}/{len(loader)}, Time to load data: {toc - tic}")
        with torch.no_grad():
            tic = time.time()
            # lizx added
            model.eval()
            ...
            if features.shape[1] < target_feature_dim:
                ...
                features = np.concatenate(
                    [features, np.zeros((features.shape[0], target_feature_dim - features.shape[1]))], axis=1
                )
            ...
            mode = 'a'
            toc = time.time()
            print(f"batch {count}/{len(loader)}, computation time {toc - tic}")
            tic = time.time()
    return output_path
...
if __name__ == '__main__':
    ...
    for bag_candidate_idx in rg:
        ...
        if not args.no_auto_skip and f'{slide_id}.pt' in dest_files:
            ...
            continue
        if (bags_dataset.df.set_index("slide_id").loc[slide_id, "process"].item() != 0):
            print(f"skipped in process list: {slide_id}")
            continue

        output_path = os.path.join(args.feat_dir, 'h5_files', bag_name)
        time_start = time.time()
        wsi = pyvips.Image.new_from_file(slide_file_path)
        try:
        ...
        if output_file_path is not None:
            ...
            torch.save(features, os.path.join(args.feat_dir, 'pt_files', f'{bag_base}.pt'))
        else:
