...
import torch
import yaml
from datasets.dataset_generic import Generic_MIL_Dataset, Generic_SlideGraph_Dataset
from utils.config_utils import parse_args
from utils.core_utils import eval, train
...
def check_and_create_dirs(args):
    ...
    if args.mode == "train":
        _extracted_from_check_and_create_dirs_9(args)
...
# TODO Rename this here and in `check_and_create_dirs`
def _extracted_from_check_and_create_dirs_9(args):
    # prepare dirs
...
def print_and_save_parameters(args, print_parameters=True, save_parameters=True):
    ...
    if save_parameters:
        d = {k: v.__dict__ if isinstance(v, argparse.Namespace) else v for k, v in args.__dict__.items()}
...
def main_train(args):
    ...
    train_dataset, val_dataset, test_dataset = dataset.return_splits(
        from_id=False, csv_path=f'{args.split_dir}/splits_{args.fold}.csv'
    )
...
def main_evaluate(args):
    ...
    if datasets_id[args.eval_split] < 0:
        ...
        csv_path = f'{args.associated_train_dir}/config/splits.csv'
    ...
    with open(os.path.join(args.eval_dir, f'EVAL--summary-fold-{args.fold}.yaml'), 'w') as f:
        yaml.dump(yaml_metric_dict, f)
    with open(os.path.join(args.eval_dir, f'EVAL--summary-fold-{args.fold}.pkl'), 'wb') as f:
...
def preprocess_args(args):
    ...
    if args.mode == "evaluate":
        ...
        elif not args.associated_train_dir and args.associated_train_root_dir:
