...
import pandas as pd
import torch
from utils.utils import TensorNamespace, device, print_network, normalize8, get_split_loader, get_optim, calculate_error, get_simple_loader
import os
import h5py
...
class EarlyStoppingModelSaver:
    ...
    def __call__(
        ...
        if self.best_score is None:
            ...
            self.save_best_checkpoint(
                val_loss, epoch, model, routine_ckpt_dir, best_ckpt_name)
    ...
    def save_best_checkpoint(self, val_loss, epoch, model, routine_ckpt_dir, best_ckpt_name):
        ...
        if self.verbose:
            print(
                f'Validation metric decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Linking current best model ...')
...
def get_heatmap(
    ...
    **kwargs,
    ...
    return wsi.visHeatmap(A, coords, **default_heatmap_params)
...
def visualize_examples(
    ...
    for slide_id in slide_ids:
        ...
        pt_fp = os.path.join(features_dir, "pt_files", f"{slide_id}.pt")
        h5_fp = os.path.join(features_dir, "h5_files", f"{slide_id}.h5")
        ...
        with torch.no_grad():
            ...
            for cl in range(len(A_all)):
                png_percnorm_dir = os.path.join(
                    slide_save_dir, "class%02d" % (cl), "png_percnorm")
                png_percnorm_fp = os.path.join(
                    png_percnorm_dir, "%s_class%02d_percnorm_epoch%03d.png" % (slide_id, cl, epoch))
                png_nopercnorm_dir = os.path.join(
                    slide_save_dir, "class%02d" % (cl), "png_nopercnorm")
                png_nopercnorm_fp = os.path.join(
                    png_nopercnorm_dir, "%s_class%02d_nopercnorm_epoch%03d.png" % (slide_id, cl, epoch))
                overlay_dir = os.path.join(
                    slide_save_dir, "class%02d" % (cl), "png_overlay")
                overlay_fp = os.path.join(
                    overlay_dir, "%s_class%02d_overlay_epoch%03d.png" % (slide_id, cl, epoch))
                mat_percnorm_dir = os.path.join(
                    slide_save_dir, "class%02d" % (cl), "mat_percnorm")
                mat_percnorm_fp = os.path.join(
                    mat_percnorm_dir, "%s_class%02d_percnorm_epoch%03d.mat" % (slide_id, cl, epoch))
                mat_nopercnorm_dir = os.path.join(
                    slide_save_dir, "class%02d" % (cl), "mat_nopercnorm")
                mat_nopercnorm_fp = os.path.join(
                    mat_nopercnorm_dir, "%s_class%02d_nopercnorm_epoch%03d.mat" % (slide_id, cl, epoch))
                ...
                im_arr = get_heatmap(
                    wsi, A_all[cl], coords, return_overlay=True, convert_to_percentiles=True, return_PIL=False)
                ...
                im_arr = get_heatmap(
                    wsi, A_all[cl], coords, return_overlay=True, convert_to_percentiles=False, return_PIL=False)
                im_cmap = normalize8(im_arr, "jet")
                im = Image.fromarray(im_cmap)
                ...
                im = get_heatmap(
                    wsi, A_all[cl], coords, return_overlay=False, convert_to_percentiles=True, return_PIL=True)
                im.save(overlay_fp)
...
class SummaryWriter(tensorboardX.SummaryWriter):
    ...
    def add_scalar(self, tag, scalar_value, global_step):
        ...
        print(f"{tag},{float(scalar_value)},{int(global_step)}",
              file=self.txt_file)
        self.txt_file.flush()
...
def initiate_model(args, ckpt_path=None):
    ...
    if ckpt_path is not None:
        ...
        def to_qkv_patch(ckpt):
            if (
                args.model_type in ["vit_aggr"]
                and 'transformer.layers.0.0.fn.to_qkv.weight' in ckpt
            ):
                ...
                to_k_weight = to_qkv_weight[per_slot_size:2 *
                                            per_slot_size, :]
                to_v_weight = to_qkv_weight[2 *
                                            per_slot_size:3 * per_slot_size, :]
...
def train(datasets, cur, args):
    ...
    save_splits(datasets, ['train', 'val', 'test'],
                os.path.join(args.config_dir, 'splits.csv'))
    print('Done!')
    ...
    if args.model_type in ["graph_vit_aggr"]:
        ...
        train2_loader = get_split_loader(
            train_split, testing=args.testing, batch_size=None, collate_fn=collate_slide_graph)
        val_loader = get_split_loader(
            val_split, testing=args.testing, batch_size=None, collate_fn=collate_slide_graph)
        test_loader = get_split_loader(
            test_split, testing=args.testing, batch_size=None, collate_fn=collate_slide_graph)
    ...
    if args.early_stopping:
        # lizx:
        early_stopping = EarlyStoppingModelSaver(
            patience=float("inf"), stop_epoch=150, verbose=True)

    else:
        early_stopping = None
    print('Done!')

    validate(cur, 0, model, train2_loader, args.n_classes, "train2", data_threshold=args.eval_data_threshold,
             early_stopping=None, writer=writer, loss_fn=loss_fn, train_dir=args.train_dir)

    for epoch in range(1, args.max_epochs + 1):
        train_loop(epoch, model, train_loader, optimizer, args.n_classes,
                   writer, loss_fn, data_threshold=args.train_data_threshold)
        validate(cur, epoch, model, train2_loader, args.n_classes, "train2", data_threshold=args.eval_data_threshold,
                 early_stopping=None, writer=writer, loss_fn=loss_fn, train_dir=args.train_dir)
    ...
    results_dict, metric_dict, df, acc_logger = summary(
...
def validate(
    cur, epoch, model, loader, n_classes, set_name, early_stopping=None,
    writer=None, loss_fn=None, train_dir=None, data_threshold=None
):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    acc_logger = Accuracy_Logger(n_classes=n_classes)
    ...
    if writer:
        writer.add_scalar(f'{set_name}/loss', val_loss, epoch)
        writer.add_scalar(f'{set_name}/auc', auc, epoch)
        writer.add_scalar('%s/error' % (set_name), val_error, epoch)

    print(f'\n{set_name}, loss: {val_loss:.4f}, error: {val_error:.4f}, auc: {auc:.4f}')
    for i in range(n_classes):
        acc, correct, count = acc_logger.get_summary(i)
        print(f'class {i}: acc {acc}, correct {correct}/{count}')
...
def eval(dataset, args, ckpt_path):
    ...
    if args.model_type in ["graph_vit_aggr"]:
        ...
        loader = get_simple_loader(
            dataset, batch_size=None, collate_fn=collate_slide_graph)
    else:
        loader = get_simple_loader(dataset)

    patient_results, metric_dict, df, _ = summary(
        model, loader, args.n_classes, data_threshold=args.eval_data_threshold)
    if args.n_classes == 2:
        print('test_accuracy: ', metric_dict["eval_scores"]["accuracy"])
        print('auc: ', metric_dict["eval_scores"]["auc_roc"])
    else:
        print('test_accuracy: ', metric_dict["eval_scores"]["accuracy"])
        print('auc (class_micro): ',
              metric_dict["eval_scores"]["auc_roc_class_micro"])
        print('auc: (class_macro)',
              metric_dict["eval_scores"]["auc_roc_class_macro"])
...
def summary(model, loader, n_classes, data_threshold=None, return_intermediates=list()):
    ...
    for batch_idx, (data, label) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            if type(ret_val) == tuple:
                ...
                logits, Y_prob, Y_hat, intermediates_tensor = ret_val["logits"], ret_val[
                    "Y_prob"], ret_val["Y_hat"], ret_val["intermediates_tensor"]
    ...
    for ts_nm in return_intermediates:
        intermediates_numpy_dict[ts_nm] = np.concatenate(
            [getattr(ns, ts_nm) for ns in intermediates_tensor_lists])
...
def compute_multiclass_metrics(all_labels, all_probs, n_classes, return_extended=False):
    ...
    if len(np.unique(all_labels)) == 1:
        ...
        cf_matrix = confusion_matrix(all_labels, all_pred, labels=[
