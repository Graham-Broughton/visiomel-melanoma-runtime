...
class DNADS(torch.utils.data.Dataset):
    def __init__(self, df, CFG, is_train):
        self.df = df
        self.config = CFG
    ...
    def __getitem__(self, index):
        ...
        img = read_img(item.filename, page=self.config.page)
        img, _ = get_tiles(
            img,
        ...
        label = np.zeros(self.config.num_classes, dtype="float32")
        label[: item[self.config.target].values] = 1.0
...
def get_loaders(CFG, test_only=False):
    train_gps, dev_gps = get_data_groups(CFG)
    batch_size = CFG.batch_size
    num_workers = min(batch_size, CFG.workers)

    dev_ds = DNADS(dev_gps, CFG, is_train=False)
    dev_sampler = None
    if CFG.distributed:
    ...
    train_ds = DNADS(train_gps, CFG, is_train=True)
