...
def apply_image_filters(np_img, slide_name=None, info=None, save=False, display=False):
    ...
    save_display(save, display, info, rgb_no_blue_pen,
                 slide_name, 6, "No Blue Pen", "rgb-no-blue-pen")
    ...
    mask_remove_small = filter_remove_small_objects(
        mask_gray_green_pens, min_size=500, output_type="bool", overmask_thresh=99.99)
...
def save_display(save, display, info, np_img, slide_name, filter_num, display_text, file_text,
    ...
    if info is not None:
        info[slide_name] = (slide_name, filter_num,
                            display_text, file_text, mask_percentage)
...
def apply_filters_to_image_list(names, save, display, ress=None):
    ...
    Args:
      image_name_tuple: List of image numbers.
      save: If True, save filtered images.
      display: If True, display filtered images to screen.
    Returns:
