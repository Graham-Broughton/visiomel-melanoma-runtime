...
def generate_top_tile_summaries(tile_sum, np_img, display=True, save_summary=False, show_top_stats=True,
    ...
    if border_all_tiles:
        for t in tile_sum.tiles:
            ...
            tile_border(draw, t.r_s + z, t.r_e + z, t.c_s,
                        t.c_e, border_color, border_size=1)
...
def summary_stats(tile_summary):
    ...
    return "Original Dimensions: %dx%d\n" % (tile_summary.orig_w, tile_summary.orig_h) + \
        ...
        tile_summary.medium, tile_summary.medium /
        tile_summary.count * 100, TISSUE_LOW_THRESH,
        TISSUE_HIGH_THRESH) + \
        " %5d (%5.2f%%) tiles >0%% and <%d%% tissue\n" % (
        tile_summary.low, tile_summary.low / tile_summary.count * 100, TISSUE_LOW_THRESH) + \
        " %5d (%5.2f%%) tiles =0%% tissue" % (tile_summary.none,
                                              tile_summary.none / tile_summary.count * 100)
...
def save_tile_data(tile_summary):
    ...
    for t in tile_summary.tiles:
        line = "%d,%d,%d,%4.2f,%s,%d,%d,%d,%d,%d,%d,%4.0f,%4.2f,%4.2f,%0.4f\n" % (
            t.tile_num, t.r, t.c, t.tissue_percentage, t.tissue_quantity(
            ).name, t.c_s, t.r_s, t.c_e, t.r_e, t.c_e - t.c_s,
...
def score_tiles(slide_name, np_img=None, dimensions=None, small_tile_in_tile=False):
    ...
    for t in tile_indices:
        ...
        score, color_factor, s_and_v_factor, quantity_factor = score_tile(
            np_tile, t_p, slide_name, r, c)

        np_scaled_tile = np_tile if small_tile_in_tile else None
        tile = Tile(tile_sum, slide_name, np_scaled_tile, count, r, c, r_s, r_e,
                    c_s, c_e, t_p, color_factor, s_and_v_factor, quantity_factor, score)
...
def image_list_to_tiles(image_list, display=False, save_summary=True, save_data=True, save_top_tiles=True):
    ...
    for slide_name in image_list:
        tile_summary = summary_and_tiles(
            slide_name, display, save_summary, save_data, save_top_tiles)
...
def np_hsv_hue_histogram(h):
    ...
    np_hist = np.fromstring(canvas.get_renderer(
    ).tostring_rgb(), dtype=np.uint8).reshape(h, w, 3)
    plt.close(figure)
...
class TileSummary:
    ...
    def display_summaries(self):
        ...
        """
        summary_and_tiles(self.slide_name, display=True,
                          save_summary=False, save_data=False, save_top_tiles=False)
