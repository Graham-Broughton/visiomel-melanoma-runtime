...
# lizx: after preparing val and test, the percentage of remaining data that goes to train
...
parser.add_argument('--test_frac', type=float, default=0.1, help='fraction of labels for test (default: 0.1)')
...
if __name__ == '__main__':
    ...
    for lf in label_fracs:
        split_dir = os.path.join(args.save_dir, f'{str(args.task)}_{int(lf * 100)}')
        os.makedirs(split_dir, exist_ok=True)
        dataset.create_splits(
            k=args.k, val_num=val_num, test_num=test_num, label_frac=lf, test_same_as_val=args.test_same_as_val
        )  # sets the dataset.split_gen attribute. Each time the generator will generate the train/val/test split for one fold

        for i in range(args.k):
            dataset.set_splits()  # lizx: set the attributes: self.train_ids, self.val_ids, self.test_ids
            descriptor_df = dataset.test_split_gen(
                return_descriptor=True, allow_val_test_overlap=args.test_same_as_val
            )  # print and return the train/val/test splits
