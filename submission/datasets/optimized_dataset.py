...
class PrefetchedPathologyPairedPatchesDataset(Dataset):
    ...
    def prefetch(self):
        ...
        for i, slide_id in enumerate(slide_ids_to_load, 1):
            print(f"{i},{slide_id}")
            slide_fp = join(self.data_files_dir, "WSI", slide_id + ".tif")
            slide_fh = pyvips.Image.new_from_file(slide_fp)
...
class BatchPrioritizedDataLoader(object):
    ...
    def __next__(self):
        ...
        return self.collate_fn(list(torch_data_loader))
...
class BatchPrioritizedDataLoaderV2(object):
    ...
    def __iter__(self):
        ...
        if self.prefetch_factor:
            self.torch_data_loader = DataLoader(
                self.dataset,
                batch_size=None,
            ...
            self.torch_data_loader = DataLoader(
                self.dataset, batch_size=None, pin_memory=self.pin_memory, num_workers=self.num_workers, shuffle=False
            )
    ...
    def __next__(self):
        ...
        tic = time.time()
        fetched_data = [
            next(self.torch_data_loader_iter) for _ in range(end_idx - start_idx)
        ]
        toc = time.time()
        if self.verbose:
