...
class FeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, dropout=0.0):
        ...
        self.net = nn.Sequential(
            nn.Linear(dim, hidden_dim), GELU(), nn.Dropout(dropout), nn.Linear(hidden_dim, dim), nn.Dropout(dropout)
        )
...
class MultiheadAttention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.0, project_out=False):
        super().__init__()
    ...
    def forward(self, x, k_inp, v_inp, attn_bias=None):  # x is q_inp
        ...
        if attn_bias is not None:
            assert attn_bias.shape == dots.shape, "wrong shape: attn_bias: {} recieved, {} expected".format(
                tuple(attn_bias.shape), tuple(dots.shape)
            )
            dots = dots + attn_bias
...
class ViTAggregation(nn.Module):
    def __init__(self, n_classes, heads=4, dim_head=32, mlp_dim=128, dim=1024, depth=1, aggr="cls_token"):
        ...
        self.transformer = TransformerEncoder(dim=dim, depth=depth, heads=heads, dim_head=dim_head, mlp_dim=mlp_dim, dropout=0.0)
    ...
    def forward(self, inp, return_intermediates=list()):
        ...
        return {
            ...
            "Y_hat": Y_hat,
            "attn_list": attn_list,
            "intermediates_tensor": intermediates_tensor,
        }
...
class ViTPairAggregation(nn.Module):
    def __init__(self, heads=4, dim_head=32, mlp_dim=128, aggr="gap"):
        ...
        self.transformer_encoder = TransformerEncoder(
            dim=dim, depth=1, heads=heads, dim_head=dim_head, mlp_dim=mlp_dim, dropout=0.0
        )
...
class GraphViTAggregation(nn.Module):
    def __init__(
        ...
        self.transformer = TransformerEncoder(
            dim=hidden_dim, depth=1, heads=self.heads, dim_head=dim_head, mlp_dim=mlp_dim, dropout=0.0
        )
        self.mlp_head = nn.Sequential(nn.LayerNorm(hidden_dim), nn.Linear(hidden_dim, n_classes))
