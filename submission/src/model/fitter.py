...
class HybridFitter:
    ...
    def resume_checkpoint(self):
        ...
        self.writer['meta'].info(
            "Loading model checkpoints ... Epoch is %s" % ckp['epoch'])
        self.model.load_state_dict(ckp['state_dict_model'])
        self.optimizers['adam'].load_state_dict(
            ckp['state_dict_optimizer_adam'])
        self.optimizers['sgd'].load_state_dict(ckp['state_dict_optimizer_sgd'])
        self.schedulers['adam'].load_state_dict(
            ckp['state_dict_scheduler_adam'])
    ...
    def train(self, epoch=0):
        ...
        for i, sample in enumerate(self.dataloaders['train']):
            ...
            nbatches = train_imgs.size(
                0) // (self.args.num_patches * self.args.num_crops)  # batch size
            ...
            with torch.set_grad_enabled(True):
                # currently only tested num_crops=1
                ...
                if self.need_permute:
                    train_targets = train_targets.view(
                        nbatches, self.args.num_patches, -1)[:, 0, :]
                train_loss = self.criterion.calculate(
...
#                 self.optimizers['sgd'].step()
    ...
    def evaluate(self, epoch=0):
        ...
        for i, sample in enumerate(tqdm(self.dataloaders['val'])):
            ...
            val_imgs = reshape_img_batch(
                val_imgs, self.args.crop_size, self.need_permute)

            nbatches = val_imgs.size(
                0) // (self.args.num_val * self.args.num_crops)

            # forward
            with torch.set_grad_enabled(False):
                val_preds = self.model(
                    val_imgs, self.args.num_val*self.args.num_crops)['pred']
                if self.args.config is None:
                    val_targets = val_targets.view(
                        nbatches, self.args.num_val, -1)[:, 0, :]
                    val_ids = val_ids.view(
                        nbatches, self.args.num_val, -1)[:, 0, :]
                eval_v.update(
                    {
                        "id": val_ids,
        ...
        save_path = os.path.join(
            "predictions", self.model_name, "%04d.csv" % epoch)
        os.makedirs(os.path.dirname(save_path),
                    exist_ok=True)  # FIXME: PATHLIB
    ...
    def save_checkpoint(
        ...
        if epoch % save_freq == 0:
            ...
            shutil.copy(epoch_output_path, Path(checkpoints_folder) /
                        ("%04d.pt" % epoch))  # FIXME F STRING
