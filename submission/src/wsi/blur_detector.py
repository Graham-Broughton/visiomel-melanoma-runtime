...
class BlurDetector(object):
    def __init__(
        ...
        self.t = Timer(name="Blur Detector")
        self.t.start()
    ...
    def createScalePyramid(self):
        return [(2 ** (self.scale_start + i)) - 1 for i in range(self.num_scales)]
    ...
    def computeImageGradientMagnitude(self, img):
        ...
        return np.sqrt(__sobelx**2.0 + __sobely**2.0)
    ...
    def detectBlur(self, img):
        ...
        with Timer(text="gaussian blur1: {:0.4f} seconds"):
            InputImageGaus = cv2.GaussianBlur(img, (3, 3), sigmaX=0.5, sigmaY=0.5)
        with Timer(text="grad mag: {:0.4f} seconds"):
        ...
        with Timer(text="DCT matrices: {:0.4f} seconds"):
            self.__createDCT_Matrices()

        # Create Frequency Labels at all the scalesv
        with Timer(text="freq bands: {:0.4f} seconds"):
        ...
        total_num_points = len(
            list(
                range(
                    int(max(self.scales) / 2),
                    rows - int(max(self.scales) / 2),
        ...
        with Timer(text="big loop: {:0.4f} seconds"):
            n, iter = 0, 0
            for i in tqdm(range(int(max(self.scales) / 2), rows - int(max(self.scales) / 2), self.downsampling_factor)):
                ...
                for j in range(int(max(self.scales) / 2), cols - int(max(self.scales) / 2), self.downsampling_factor):
                    ...
                    for ind, curr_scale in enumerate(self.scales):
                        Patch = __padded_image[
                            i - int(curr_scale / 2) : i + int(curr_scale / 2) + 1,
                            j - int(curr_scale / 2) : j + int(curr_scale / 2) + 1,
                        ]
                    ...
                    # Find the first `total_num_layers` smallest values in all the high frequency components
                    # - we must not sort the entire array since that is very inefficient
        ...
        with Timer(text="normalize L matrix: {:0.4f} seconds"):
            ...
            max_val = 0
            min_val = 99999
            for ind1d, (i, j) in enumerate(itertools.product(range(n), range(m))):
                T_max[i][j] = max(L[ind1d, :])
                max_val = max(max_val, T_max[i][j])
                min_val = min(min_val, T_max[i][j])

        # Final Map and Post Processing
        with Timer(text="final map & psot processing: {:0.4f} seconds"):
        ...
        with Timer(text="resize & match matrix: {:0.4f} seconds"):
        ...
        with Timer(text="resize to original res: {:0.4f} seconds"):
            final_map = cv2.resize(final_map, (ori_cols, ori_rows))

        # normalize the map
        with Timer(text="final normalization: {:0.4f} seconds"):
            final_map = final_map / np.max(final_map)
        self.t.text = "Total time : {:0.4f}"
        self.t.stop()
        return final_map
