...
def get_split_loader(split_dataset, training=False, testing=False, weighted=False, batch_size=1, collate_fn=collate_MIL):
    ...
    if testing:
        ...
        loader = DataLoader(
            split_dataset, batch_size=batch_size, sampler=SubsetSequentialSampler(ids), collate_fn=collate_fn, **kwargs
        )
        ...
        if weighted:
            ...
            loader = DataLoader(
                split_dataset, batch_size=batch_size, sampler=RandomSampler(split_dataset), collate_fn=collate_fn, **kwargs
            )
    else:
        loader = DataLoader(
            split_dataset, batch_size=batch_size, sampler=SequentialSampler(split_dataset), collate_fn=collate_fn, **kwargs
        )
...
def generate_split(
    cls_ids, val_num, test_num, samples, n_splits=5, seed=7, label_frac=1.0, custom_test_ids=None, test_same_as_val=False
):
    ...
    for _ in range(n_splits):
        ...
        for c in range(len(val_num)):  # lizx: for each class
            ...
            elif custom_test_ids is None:  # sample test split
...
def normalize8(I, cmap=None, mn=None, mx=None):
    ...
    if mx is None:
        mx = I.max()
    mx -= mn
    I = (I - mn) / mx if mx > 0 else np.zeros_like(I, dtype=np.float32)
    if cmap is not None:
        cm = plt.get_cmap('jet')
        I = cm(I)
...
class TensorNamespace(argparse.Namespace):
    ...
    def size(self, dim=None):
        return self.x.size(dim) if dim is not None else self.x.size()
