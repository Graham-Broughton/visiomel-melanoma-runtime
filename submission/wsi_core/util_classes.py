...
class Mosaic_Canvas(object):
    def __init__(self, patch_size=256, n=100, downscale=4, n_per_row=10, bg_color=(0, 0, 0), alpha=-1):
        ...
        if alpha < 0:
            ...
            canvas = Image.new(size=(w, h), mode="RGBA",
    ...
    def paste_patch(self, patch):
        ...
        self.canvas.paste(
            patch.resize((self.downscaled_patch_size, self.downscaled_patch_size)),
            tuple(self.coord),
        )
...
class isInContourV2(Contour_Checking_fn):
    ...
    def __call__(self, pt):
        return 1 if cv2.pointPolygonTest(
            self.cont, (pt[0] + self.patch_size // 2, pt[1] + self.patch_size // 2), False
        ) >= 0 else 0
...
class isInContourV3_Easy(Contour_Checking_fn):
    ...
    def __call__(self, pt):
        ...
        if self.shift > 0:
            all_points = [(float(center[0] - self.shift), float(center[1] - self.shift)),
