...
def isWhitePatch(patch, satThresh=5):
    ...
    return np.mean(patch_hsv[:, :, 1]) < satThresh


def isBlackPatch(patch, rgbThresh=40):
    return bool(np.all(np.mean(patch, axis=(0, 1)) < rgbThresh))


def isBlackPatch_S(patch, rgbThresh=20, percentage=0.05):
    num_pixels = patch.size[0] * patch.size[1]
    return (
        np.all(np.array(patch) < rgbThresh, axis=(2)).sum()
        > num_pixels * percentage
    )
...
def savePatchIter_bag_hdf5(patch):
    ...
    file_path = f'{os.path.join(save_path, name)}.h5'
...
def initialize_hdf5_bag(first_patch, save_coord=False):
    ...
    if save_coord:
        coord_dset = file.create_dataset('coords', shape=(
            1, 2), maxshape=(None, 2), chunks=(1, 2), dtype=np.int32)
        coord_dset[:] = (x, y)
...
def top_k(scores, k, invert=False):
    return scores.argsort()[:k] if invert else scores.argsort()[::-1][:k]
...
def sample_rois(
    scores, coords, k=5, mode='range_sample', seed=1, score_start=0.45, score_end=0.55, top_left=None, bot_right=None
):
    ...
    if mode == 'range_sample':
        sampled_ids = sample_indices(
            scores, start=score_start, end=score_end, k=k, convert_to_percentile=False, seed=seed)
    elif mode == 'topk':
    ...
    return {'sampled_coords': coords, 'sampled_scores': scores}


def DrawGrid(img, coord, shape, thickness=2, color=(0, 0, 0, 255)):
    cv2.rectangle(img, tuple(np.maximum([0, 0], coord - thickness // 2)), tuple(
        coord - thickness // 2 + np.array(shape)), (0, 0, 0, 255), thickness=thickness)
    return img
...
def DrawMap(canvas, patch_dset, coords, patch_size, indices=None, verbose=1, draw_grid=True):
    ...
    for idx in range(total):
        if verbose > 0 and idx % ten_percent_chunk == 0:
            print(f'progress: {idx}/{total} stitched')
        ...
        canvas[coord[1]:coord[1] + patch_size[1], coord[0]:coord[0] + patch_size[0],
...
def StitchPatches(hdf5_file_path, downscale=16, draw_grid=False, bg_color=(0, 0, 0), alpha=-1):
    ...
    if alpha < 0 or alpha == -1:
        ...
        heatmap = Image.new(size=(w, h), mode="RGBA",
                            color=bg_color + (int(255 * alpha),))

    heatmap = np.array(heatmap)
    heatmap = DrawMap(heatmap, dset, coords, downscaled_shape,
                      indices=None, draw_grid=draw_grid)
...
def StitchCoords(hdf5_file_path, wsi_object, downscale=16, draw_grid=False, bg_color=(0, 0, 0), alpha=-1):
    ...
    heatmap = np.array(heatmap)
    heatmap = DrawMapFromCoords(heatmap, wsi_object, coords,
                                patch_size, vis_level, indices=None, draw_grid=draw_grid)
...
def SamplePatches(coords_file_path, save_file_path, wsi_object,
    ...
    if verbose > 0:
        ...
        print(
            f'in .h5 file: patch size: {h5_patch_size}x{h5_patch_size} patch level: {h5_patch_level}'
        )
    ...
    np.random.seed(seed)
    indices = np.random.choice(np.arange(len(coords)), min(
        len(coords), sample_num), replace=False)

    target_patch_size = np.array([patch_size, patch_size])

    if custom_downsample > 1:
        target_patch_size = (
            np.array([patch_size, patch_size]) / custom_downsample).astype(np.int32)

    if stitch:
        canvas = Mosaic_Canvas(
            patch_size=target_patch_size[0], n=sample_num, downscale=4, n_per_row=10, bg_color=(0, 0, 0), alpha=-1)
    else:
        canvas = None
    ...
    for idx in indices:
        ...
        asset_dict = {'imgs': np.array(
            patch)[np.newaxis, ...], 'coords': coord}
